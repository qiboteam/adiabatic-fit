...
import qibo 
from qibo.optimizers.minimizers import ScipyMinimizer, ParallelBFGS
from qibo.optimizers.heuristics import BasinHopping, CMAES
...
def train_adiabatic_evolution(
    ...
    def plot_for_energies(parameters, label="", true_law=None, title="", save=""):
        ...
        plt.plot(xarr, -np.array(energies), label=label, color='purple', lw=2, alpha=0.8)
        plt.plot(xarr[idx_training], -np.array(cdf_training), 'o', label="Training points", color='orange', alpha=0.85, markersize=8)
        if true_law != None:
    ...
    def optimize(force_positive=False, target=5e-2, max_iterations=50000, max_evals=500000, initial_p=None):
        ...
        start = time.time()
        options = {"ftol": 1e-16}
        opt = ParallelBFGS(loss=loss_evaluation, initial_parameters=initial_p)
        
        result = opt.fit()
        end = time.time()

        print(f"Required training time: {end-start}\nReached loss: {result[0]}")
    ...
    plot_for_energies(
        init_params, 
        label="Initial state", 
        title="Not trained evolution", 
        save=f"initial_{variable_id}.png"
        )
    ...
    plot_for_energies(
        best_params, 
        label="Initial state", 
        title="Trained evolution", 
        save=f"final_{variable_id}.png"
        )
