...
def generate_schedule(params, schedule="poly"):
    """Generates the scheduling used into the adiabatic evolution."""
    nparams = len(params)
    def poly(t):
...
def generate_adiabatic(params, h0, h1, obs_target, dt=1e-1, solver="exp"):
    """Generates the adiabatic evolution object."""
    energy = callbacks.Energy(obs_target)
    #same scheduling function
    schedule, _ = generate_schedule(params, schedule="poly")
    return models.AdiabaticEvolution(
        h0, h1, schedule, dt=1e-1, solver="exp", callbacks=[energy],
    ), energy
...
def perform_adiabatic(params, finalT, h0, h1, obs_target, dt=1e-1, solver="exp"):
    """Returns energy callback as an array"""
    evolution, energy = generate_adiabatic(params, h0, h1, obs_target, dt=1e-1, solver="exp")
